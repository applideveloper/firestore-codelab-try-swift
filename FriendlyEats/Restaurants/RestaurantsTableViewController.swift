...
class RestaurantsTableViewController: UIViewController, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let sampleQuery = Firestore.firestore().collection("restaurants").limit(to: 50)
        
        sampleQuery.getDocuments { (snapshot, error) in
            if let _ = error {
            ...
            for restaurantDocument in allDocuments {
                if let newRestaurant = Restaurant.init(document: restaurantDocument) {
